...
void ACRPawn::Tick(float DeltaTime)
{
	...
	{
		//
		FVector Impulse = FVector(0.0f, ForwardForce, 0.0f);
...
//
void ACRPawn::OnHit(UPrimitiveComponent * MyComp, AActor * Other, UPrimitiveComponent * OtherComp,
...
void ACRPawn::MoveRight(float AxisValue)
{
	if (!bGameEnded)
	{
		FVector Impulse = FVector(SideForce  * AxisValue,0.0f, 0.0f);
		Cube->AddImpulse(Impulse, "", true);
	}
}
